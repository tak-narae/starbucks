const QtyCalc = (setIsQtyCalcRunning, cartList, setCartList) => {
  ...
  spinners.forEach((spinner) => {
    ...
    const updatePrice = () => {
      ...
      if (
        document.querySelector("#container").classList.contains("prd__detail")
      ) {
        const totalPriceEl = spinner
          .closest(".total_item")
          .querySelector(".total_price");
        const originPrice = parseInt(
          document
            .querySelector('[class^="prd__detail"] .prd_item .info_cont .price')
            .textContent.replaceAll(",", "")
        );
        // console.log(originPrice, "*" ,qtyEl.value, "=" ,totalPriceEl.textContent);
        totalPriceEl.textContent =
          (originPrice * qtyEl.value).toLocaleString() + "원";
      }

      if (
        document.querySelector("#container").classList.contains("order__cart")
      ) {
