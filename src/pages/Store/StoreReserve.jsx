...
const StoreReserve = () => {
  ...
  useEffect(() => {
    ...
    const starbucksMarker = {
      url: "/db/images/pin_general_R.png", // Reserve 매장 아이콘
      scaledSize: new window.google.maps.Size(30, 48),
    };

    // R 매장이 있을 경우 줌 아웃 (기본 줌 레벨보다 낮게 설정)
    const hasReserveStore = stores.some((store) => store.name.includes("R"));
    if (hasReserveStore) {
      newMap.setZoom(12); // R 매장이 있으면 줌 아웃
    }

    // stores가 배열이면 매장 필터링 후 마커 추가
    if (Array.isArray(stores) && stores.length > 0) {
      stores
        .filter((store) => store.name.includes("R")) // "R"이 포함된 매장만 필터링
        .forEach((store) => {
